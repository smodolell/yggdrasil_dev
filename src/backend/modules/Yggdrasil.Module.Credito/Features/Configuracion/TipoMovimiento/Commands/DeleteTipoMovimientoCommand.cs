...
) : ICommandHandler<DeleteTipoMovimientoCommand, Result>
{
    ...
    public async Task<Result> HandleAsync(DeleteTipoMovimientoCommand message, CancellationToken cancellationToken = default)
    {
        try
        {
            var oTipoMovimiento = await _context.FI_TipoMovimiento.SingleOrDefaultAsync(r => r.Id == message.TipoMovimientoId, cancellationToken);
            if (oTipoMovimiento == null)
            {
                return Result.NotFound("No se encontró la TipoMovimiento.");
            }
            _context.FI_TipoMovimiento.Remove(oTipoMovimiento);
