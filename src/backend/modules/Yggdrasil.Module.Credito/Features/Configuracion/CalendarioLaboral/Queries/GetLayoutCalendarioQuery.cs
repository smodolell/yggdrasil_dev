...
public class GetLayoutCalendarioQuery : IQuery<Result<byte[]>>
{
}

internal class GetLayoutCalendarioQueryHandler(
) : IQueryHandler<GetLayoutCalendarioQuery, Result<byte[]>>
{
    public async Task<Result<byte[]>> HandleAsync(GetLayoutCalendarioQuery request, CancellationToken cancellationToken = default)
    {
        try
        {
            var bytes = GenerarLayoutCalendarioLaboral();
    ...
    public static byte[] GenerarLayoutCalendarioLaboral()
    {
